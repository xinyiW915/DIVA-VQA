...
def extract_csv2metadata(df, video_type):
    ...
    if video_type == 'lsvq':
        for i in range(len(df)):
            ...
            # video_path = f"/media/on23019/server/LSVQ/{df['name'][i]}.mp4"
        ...
        for i in range(len(df)):
            video_path = f"D:/video_dataset/ugc-dataset/youtube_ugc/original_videos/all_h264/{df['vid'][i]}_crf_10_ss_00_t_20.0.mp4"
            if os.path.exists(video_path):
                vid_list.append(f"{df['vid'][i]}_crf_10_ss_00_t_20.0")
                mos_list.append(df['MOS full'][i])
...
if __name__ == '__main__':
    ...
    if video_type == 'lsvq':
        ...
        # df = pd.read_csv(f"/home/xinyi/video_dataset/LSVQ/LSVQ_whole_{set_name}.csv")
    ...
    # YOUTUBE_UGC_h264
        ...
        df = pd.read_excel(f"D:/video_dataset/ugc-dataset/youtube_ugc/original_videos/original_videos_MOS_for_YouTube_UGC_dataset.xlsx")
        df_new = extract_csv2metadata(df, video_type)
        print(df_new)
        video_type = f'/{video_type.upper()}_H264'
